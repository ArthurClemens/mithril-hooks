...
describe("update rules", () => {
  ...
  it("empty array: should run the effect only once with mount only", () => {
    ...
    cy.get("[data-test-id=RunCountOnMount] [data-test-id=effectRunCount]").should("contain", "effect called: 1");
    cy.get("[data-test-id=RunCountOnMount] [data-test-id=renderRunCounts]").should("not.contain", "render called: 1");
  });
  ...
  it("array with variable: should run the effect only after variable change", () => {
    ...
    cy.get("[data-test-id=RunCountOnChange] [data-test-id=renderRunCounts]").should("not.contain", "render called: 2");
  });
  ...
  it("no array: should run the effect at each render", () => {
    ...
    cy.get("[data-test-id=RunCountOnRender] [data-test-id=renderRunCounts]").should("not.contain", "render called: 3");
  });
